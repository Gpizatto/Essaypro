...
async def get_essay(essay_id: str, current_user: dict = Depends(get_current_user)):
    ...
    import asyncio
    student, prompt = await asyncio.gather(
        db.users.find_one({"_id": ObjectId(essay["student_id"])}, {"_id": 0, "name": 1}),
        db.prompts.find_one({"id": essay["prompt_id"]}, {"_id": 0, "title": 1}),
    )
    if student:
...
async def startup_event():
    ...
    await db.essays.create_index("id", unique=True)        # busca por essay_id (string)
    ...
    await db.corrections.create_index("id", unique=True)   # busca por correction id
    ...
    await db.prompts.create_index("id", unique=True)        # busca por prompt_id (string)
